#!/bin/bash

echo "🚀 UNAMI MOMENTS PRODUCTION SETUP"
echo "=================================="

echo -e "\n1. 🔐 SUPERADMIN SETUP REQUIRED"
echo "==============================="
echo "The admin dashboard shows 401 Unauthorized because no superadmin exists."
echo ""
echo "MANUAL STEPS REQUIRED:"
echo "1. Go to Supabase SQL Editor: https://app.supabase.com/project/arqeiadudzwbmzdhqkit/sql"
echo "2. Execute this SQL:"
echo ""
echo "-- Create superadmin user"
echo "INSERT INTO public.admin_roles (user_id, role, created_by)"
echo "VALUES ('00000000-0000-0000-0000-000000000000', 'superadmin', '00000000-0000-0000-0000-000000000000')"
echo "ON CONFLICT (user_id) DO UPDATE SET"
echo "  role = EXCLUDED.role,"
echo "  updated_at = NOW();"
echo ""
echo "3. Verify with: SELECT * FROM admin_roles WHERE role = 'superadmin';"
echo ""

echo -e "\n2. 🌐 DEPLOYMENT STATUS"
echo "======================"
echo "✅ Vercel (Landing): https://moments.unamifoundation.org"
echo "✅ Railway (Admin/API): https://moments-api.unamifoundation.org"
echo "✅ WhatsApp API: +27 65 829 5041"
echo ""

echo -e "\n3. 🔧 CURRENT ISSUES RESOLVED"
echo "============================"
echo "✅ Fixed Vercel 500 serverless function errors"
echo "✅ Added proper admin-dashboard.html redirects"
echo "✅ Separated landing page (Vercel) from admin (Railway)"
echo "⚠️  PENDING: Superadmin user creation in Supabase"
echo ""

echo -e "\n4. 📋 NEXT STEPS"
echo "==============="
echo "1. Create superadmin user in Supabase (see SQL above)"
echo "2. Test admin access: https://moments-api.unamifoundation.org/admin-dashboard.html"
echo "3. Verify WhatsApp webhook: https://moments-api.unamifoundation.org/health"
echo "4. Test community join: https://wa.me/27658295041?text=START"
echo ""

echo -e "\n5. 🎯 ADMIN DASHBOARD ACCESS"
echo "==========================="
echo "URL: https://moments-api.unamifoundation.org/admin-dashboard.html"
echo "Login: Use any email/password (JWT-based auth)"
echo "Note: Will show 401 until superadmin user is created"
echo ""

echo "✅ Setup script complete. Execute the Supabase SQL to enable admin access."